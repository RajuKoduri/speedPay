...
@Component({
  selector: 'app-player-manual-deposit',
  templateUrl: './player-manual-deposit.component.html',
  styleUrls: ['./player-manual-deposit.component.css']
})
export class PlayerManualDepositComponent implements OnInit {
  PlayerName: any;
  wallettypelist: any;
  currencys: any = [];
  ...
  getPlayerVisibleCashWallets() {
    ...
    this.PlayerServiceService.getPlayerVisibleCashWallets(body).subscribe((data) => {
      ...
      for (let i = 0; i < this.visibleCahOutList.length; i++) {
        ...
        for (let m = 0; m < this.WalletList.length; m++) {
          if (this.visibleCahOutList[i].preferredWallet.lowLong == this.WalletList[m].guid.lowLong) {
            this.selectedCurrency = this.WalletList[m].guid
          }

        }
      }
      this.getPlayersBalances()
       this.TopupWalletAmount()
    })
  ...
  TopupWalletAmount() {
    ...
    console.log(this.WalletList)
    let ExchangeValue: any
    let topupWalletGuidCurrency: any
    let topupExchangeValue: any

    for (let i = 0; i < this.ExchangeValue?.length; i++) {
      if (this.SelectedOrderAmountwallet?.lowLong == this.ExchangeValue[i]?.currency?.lowLong) {
        ExchangeValue = Number(this.ExchangeValue[i].exchange)
      }

      for (let j = 0; j < this.WalletList.length; j++) {
        if (this.selectedCurrency?.lowLong == this.WalletList[j].guid.lowLong) {
          topupWalletGuidCurrency = this.WalletList[j].currency
        }
      }
      if (topupWalletGuidCurrency?.lowLong == this.ExchangeValue[i].currency.lowLong) {
        topupExchangeValue = Number(this.ExchangeValue[i].exchange)
      }
      // this.selectedCurrency
    }
    console.log("topupWalletGuidCurrency ", topupWalletGuidCurrency)
    console.log("Exchange Value", ExchangeValue)
    console.log("topupExchangeValue ", topupExchangeValue)

    this.truncatedNumber = (Number(this.OrderAmountinput) * (Number(topupExchangeValue)))
    let truncatedNumber01 = (Number(this.truncatedNumber) /( Number(ExchangeValue)))
    this.OrderAmountSum = Number(Math.floor(truncatedNumber01 * 100) / 100);
  }
  SeletedWallet() {
    this.OrderAmount01()
  }
  OrderAmount01() {
    this.TopupWalletAmount()
    // (this.OrderAmountSum) = Number(this.OrderAmountinput);
    // console.log(this.SelectedOrderAmountwallet)
    // // this.selectedCurrency
    // for (let i = 0; i < this.ExchangeValue.length; i++) {
    //   for (let j = 0; j < this.currencys.length; j++) {
    ...
    //         console.log(this.currencys[j])
    //         this.truncatedNumber = (Number(this.OrderAmountinput) * (Number(this.ExchangeValue[i].exchange)))
    ...
    //     if (this.SelectedOrderAmountwallet?.lowLong == this.ExchangeValue[i].currency.lowLong) {
    //       let truncatedNumber01 =(Number (this.truncatedNumber)) /(Number(this.ExchangeValue[i].exchange))
