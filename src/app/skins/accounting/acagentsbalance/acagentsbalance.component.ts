...
@Component({
  selector: 'app-acagentsbalance',
  templateUrl: './acagentsbalance.component.html',
  styleUrls: ['./acagentsbalance.component.css']
})
export class AcagentsbalanceComponent implements OnInit {
  ...
  exportExcel(){
    ...
    let fileName:'AgentsbalanceExeclSheet.xlsx';
    XLSX.writeFile(wb, 'AgentsBalanceExeclSheet.xlsx');
  }
  exportCsv(): void {
    let element = document.getElementById('excel_table');
    this.FileformatServiceService.exportCsv(element, "AgentsBalanceExeclSheet")
  }
  onFormSubmit() {
    ...
    this.accountingService.AgentsList(fillterbody).subscribe((res) => { this.getAgentsBalancetData(res) })
  }
  getValues(form: any){
    let Values:any ={};
  ...
  getAgentsBalancetData(data:any){
    this.dataLoader = false;
    this.agentsBalance = data.objectList;
    this.rowsOnthePage = data.objectList.length;
    this.ResultCount = data.resultCount;
    console.log(this.agentsBalance)
    if (this.ResultCount !== null) {
      this.resultcount = false;
    }
    if (this.agentsBalance != null) {
      this.dataLoader = false;
    }
    this.sumOfRow(this.agentsBalance)

    for (let i = 0; i < this.agentsBalance.length; i++) {
      for (let m = 0; m < this.wallettypelist.length; m++) {
        if(this.agentsBalance[i]?.currencys?.lowLong == this.wallettypelist[m].guid.lowLong) {
          this.agentsBalance[i].currencys = this.wallettypelist[m].value
        }
      }
    }
  }
  ...
  onClick(event: any) {
    console.log(event)
    this.subAgents = this.agentsBalance[event]
    console.log(this.subAgents)
    this.agentsBalpopup = true;
  }
  openAgentsPop() {
    this.agentsBalpopup = true;
  }
  closePopup() {
    this.agentsBalpopup = false;
  }
  sumOfRow(data:any){
    this.obj = data
