...
@Component({
  selector: 'app-acaffiliateexpenses',
  templateUrl: './acaffiliateexpenses.component.html',
  styleUrls: ['./acaffiliateexpenses.component.css']
})
export class AcaffiliateexpensesComponent implements OnInit {
  ...
  ignoreContext() {
    this.affiliateExpenses = [];
    this.dataLoader = true;
    this.affiliateExpenses = false;
  }
  ...
  exportExcel() {
    ...
    let fileName: 'AffiliateExpenseExeclSheet.xlsx';
    XLSX.writeFile(wb, 'affiliateExpenseExeclSheet.xlsx');
  }
  exportCsv(): void {
    let element = document.getElementById('excel_table');
    this.FileformatServiceService.exportCsv(element, "affiliateExpenseExeclSheet")
  }
  onFormSubmit() {
    ...
    let body = {
      ...
      "reportingPeriod": (this.filterForm.value.startDate != null && this.filterForm.value.startDate !== "") ? {
        "start": this.filterForm.value.startDate + 'T' + (this.selectedTime),
        "end": this.filterForm.value.endDate + 'T' + (this.selectedEndTime)
      } : undefined,
      "affiliateLogin": this.filterForm.value.affiliateLogin! = null ? this.filterForm.value.affiliateLogin : undefined,
    ...
    console.log(this.filterForm.value.affiliateLogin);
    console.log(this.filterForm.value.firstRecord);
    console.log(this.filterForm.value.pageSize);

    this.accountingService.AffExpList(body).subscribe((res) => { this.getAffExpenseData(res) })
  }
  getValues(form: any) {
    let Values: any = {};
  ...
  getAffExpenseData(data: any) {
    this.dataLoader = false;
    this.affiliateExpenses = data?.objectList;
    console.log(this.affiliateExpenses);
    ...
    if (this.affiliateExpenses != null) {
      this.dataLoader = false;
    }
    // this.sumOfRow(this.affiliateExpenses)

    for (let i = 0; i < this.affiliateExpenses?.length; i++) {
      for (let m = 0; m < this.wallettypelist.length; m++) {
        if (this.affiliateExpenses[i]?.currencys?.lowLong == this.wallettypelist[m].guid.lowLong) {
          this.affiliateExpenses[i].currencys = this.wallettypelist[m].value
        }
      }
    }
  }
  ...
  onClick(event: any) {
    console.log(event)
    this.subAffiliate = this.affiliateExpenses[event]
