...
export class GamesofpokerService {
  ...
  GamesConfig() {
    ...
    this.remoteGamesList = this.GamesConfigList.remoteSystemGames 
    console.log("remotegameslist", this.remoteGamesList)
    ...
    const currencyFormats = localStorage.getItem('currencyFormatsSymbol')
    if (currencyFormats) {
      this.currencyFormatsSymbol = JSON.parse(currencyFormats)
    }
    ...
    for (let i = 0; i < this.gamesListofarrays.length; i++) {
      ...
      if (this.gamesListofarrays[i].caption) {
        ...
        // }
	 if(this.gamesListofarrays[i].name.startsWith("RUMMY")){
          this.RummyGameslist.push(this.gamesListofarrays[i])
          this.RummyGamesNames.push(this.gamesListofarrays[i].name)
          this.RummyGamesCaptions.push(this.gamesListofarrays[i].caption)
        }

        if (this.gamesListofarrays[i].name.startsWith("POKER_")) {
          this.PokerGamesOnly.push(this.gamesListofarrays[i])
    ...
    return [this.filterItem, this.allGammeNamesWithCurrency, this.gamescaption,
    this.allGammeNames, this.onlyPokerGames, this.uniquegamescaption,
    this.checkboxesIndivisualGames, this.Casinofilters, this.onlycasinoGames,
    this.casinoGamesname, this.casinoGamescpt, this.uniquegamescaptionCasino,
    this.gamescaptioncasino, this.PokerGamesOnly, this.currencyFormatsSymbol,this.remoteGamesList,
    this.RummyGameslist,this.RummyGamesNames,this.RummyGamesCaptions] 
  }
  ...
  setData(data: any, tournamentId: any = null) {
    // this.data = data;
    let storageKey: any
    // localStorage.setItem('storageKey', JSON.stringify(data))
    // localStorage.setItem('storageKey', JSON.stringify({data:data,tournamentId:tournamentId}))
    sessionStorage.setItem('storageKey', JSON.stringify({ data: data, tournamentId: tournamentId }))
  }
  ...
  clearData() {
    ...
    this.onlycasinoGames = [];
    this.Casinofilters = [];
    this.onlycasinoGames = [];

    this.RummyGameslist = [];
    this.RummyGamesNames = []
    this.RummyGamesCaptions = [];
  }
