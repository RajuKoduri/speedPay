...
@Component({
  selector: 'app-affiliatepayments',
  templateUrl: './affiliatepayments.component.html',
  styleUrls: ['./affiliatepayments.component.css']
})
export class AffiliatepaymentsComponent implements OnInit {
  ...
  constructor(private Cashierservice: CashierService,private FileformatServiceService: FileformatServiceService) {
    this.filterForm = new FormGroup({
      ...
      AffiliateAccoutMask: new FormControl(),
  ...
  onFormSubmit() {
    ...
    fillterbody["webmasterStatuses"] = this.filterForm.value.Status !=null ? this.filterForm.value.Status:undefined ;
    fillterbody["paymentStates"] = this.currencyGuids,
    // fillterbody["paymentStates"] = this.filterForm.value.wallet !=null ? this.filterForm.value.wallet:undefined ;
    ...
    this.Cashierservice.getWebmasterPaymentsByFilter(fillterbody).subscribe((res) => { this.AffiliatePaymentData(res) })
}

AffiliatePaymentData(data:any){
console.log(data)
this.AffiliateData = data.objectList;
...
exportexcel(): void {
  ...
  XLSX.writeFile(wb, 'AffiliatePaymentsExcelSheet.xlsx'); 
}
exportCsv(): void {
  let element = document.getElementById('excel_table');
  this.FileformatServiceService.exportCsv(element, "AffiliatePaymentsExcelSheet")
}
